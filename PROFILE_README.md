# 👋 Hi, I'm CertainlyP

### 🔒 Security Researcher | 🎯 Threat Hunter | 📝 Detection Engineer

I focus on threat hunting, detection engineering, and security research. Passionate about building effective detection strategies and sharing knowledge with the security community.

---

## 🎯 Current Focus

- 🔍 Developing threat hunting techniques and detection rules
- 📊 Researching adversary TTPs and MITRE ATT&CK mapping
- 🛡️ Building detection engineering frameworks
- ✍️ Writing about security research and analysis

## 🛠️ Tech Stack & Tools

### Security & Analysis
![KQL](https://img.shields.io/badge/KQL-Query-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![Sigma](https://img.shields.io/badge/Sigma-Rules-FF6C37?style=flat-square)
![Splunk](https://img.shields.io/badge/Splunk-SPL-000000?style=flat-square&logo=splunk&logoColor=white)
![MITRE ATT&CK](https://img.shields.io/badge/MITRE-ATT%26CK-FF0000?style=flat-square)

### Languages & Platforms
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

---

## 📚 Blog & Research

I maintain a threat hunting research blog where I share:
- Detection techniques and rules (Sigma, KQL, SPL)
- Threat hunting methodologies
- Security tool reviews
- IoC analysis and MITRE ATT&CK mappings

**📖 Read my blog:** [Threat Hunting Research](https://certainlyp.github.io/git-playground/)

---

## 📊 GitHub Stats

![CertainlyP's GitHub stats](https://github-readme-stats.vercel.app/api?username=CertainlyP&show_icons=true&theme=dark&hide_border=true&bg_color=0d1117)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=CertainlyP&layout=compact&theme=dark&hide_border=true&bg_color=0d1117)

---

## 🎓 Interests & Expertise

```yaml
areas_of_interest:
  - Threat Hunting & Detection Engineering
  - SIEM/XDR Detection Rule Development
  - Adversary Emulation & Purple Teaming
  - Security Operations & Incident Response
  - MITRE ATT&CK Framework
  - Log Analysis & Behavioral Analytics
```

---

## 📫 Connect With Me

- 📝 Blog: [certainlyp.github.io/git-playground](https://certainlyp.github.io/git-playground/)
- 🐦 Twitter: [@YourHandle](https://twitter.com/YourHandle) *(update with your handle)*
- 💼 LinkedIn: [Your LinkedIn](https://linkedin.com/in/yourprofile) *(update with your profile)*
- 🔗 GitHub: [@CertainlyP](https://github.com/CertainlyP)

---

## 🤝 Collaboration

I'm interested in collaborating on:
- 🎯 Threat hunting research and methodology
- 🔍 Detection rule development (Sigma, KQL, SPL)
- 📊 Security data analysis projects
- 🛡️ Open-source security tools

Feel free to reach out if you want to collaborate or discuss security research!

---

<div align="center">

### 💭 Quote

*"The best defense is a good detection"*

---

![Profile Views](https://komarev.com/ghpvc/?username=CertainlyP&color=brightgreen&style=flat-square)

</div>
